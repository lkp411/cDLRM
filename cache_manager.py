...
class CacheManagerProcess(mp.Process):
	def __init__(self, args_queue, emb_tables_cpu, batch_fifo, eviction_fifo, finish_event, ndevices):
		...
		try:
			while(True):
				...
				for k, table_eviction_data in enumerate(eviction_data):
					...
					emb_tables.emb_l[k].weight.data[idxs] = (emb_tables.emb_l[k].weight.data[idxs] + embeddings) / 2 if average_on_writeback else embeddings
		except:
	...
	def run(self):
		...
		eviction_process = mp.Process(target=CacheManagerProcess.eviction_manager, args=(self.emb_tables_cpu, self.eviction_fifo, args.average_on_writeback))
		...
		for epoch in range(args.nepochs):
			for j, (X, lS_o, lS_i, T) in enumerate(cache_ld):
				...
				processed_slices = [pool.apply_async(CacheManagerProcess.process_batch_slice, args=(lS_i[:, p*num_examples_per_process :  (p+1)*num_examples_per_process], self.emb_tables_cpu)) for p in range(num_processes_needed)] 
